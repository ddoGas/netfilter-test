...
bool block_host(struct nfq_data *tb){
	unsigned char *data;
	...
	if(ipv4_hdr->ip_p != TCP)
		return false;
	int total_len = ipv4_hdr->ip_len;
	int ip_len = ipv4_hdr->ip_hl << 2;

	struct libnet_tcp_hdr* tcp_hdr = (libnet_tcp_hdr*)(data + ip_len);
	...
	char * host = strstr((char*)(data + ip_len + tcp_len), "Host: ");
	if(host==NULL)
		return false;
	
	host = host+6;
	for(int i=0;i<strlen(target_host);i++)
		if(target_host[i]!=host[i])
			return false;

	if(host[strlen(target_host)]!='\r')
		return false;
